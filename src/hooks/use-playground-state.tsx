...
// Updated context type without auth dialog properties
...
const PlaygroundStateContext = createContext<PlaygroundStateContextProps | undefined>(undefined);
...
export const PlaygroundStateProvider = ({
  ...
  useEffect(() => {
    // First check for environment variable (must be prefixed with NEXT_PUBLIC_ to be client-accessible)
    const envApiKey = process.env.NEXT_PUBLIC_OPENAI_API_KEY;

    if (envApiKey && envApiKey.length >= 1) {
      dispatch({ type: "SET_API_KEY", payload: envApiKey });
    } else {
      ...
      if (storedKey && storedKey.length >= 1) {
        ...
        console.warn("No OpenAI API key found in environment variables or localStorage");
      }
    }
